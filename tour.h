...
public:
	...
	double off_gen(int *sol_blue, int *sol_red, int *offspring, int *label_list, int best);
	void print(void);
	void report(void);
...
double candidates::off_gen(int *sol_blue, int *sol_red, int *offspring, int *label_list, int best) {
	...
	if (blue_fitness_rest > red_fitness_rest)
		select_rest = best; // 1 for red and 0 for blue if best=1, 0 for red and 1 for blue othewise.
	else
		select_rest = best ^ 1;
	...
	Grafo::Aresta *b;	  			// next edge of the vertex
	v_aux=a->_v2 ();
	i++;
